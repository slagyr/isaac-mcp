...
(defn- await-pending! [id]
  (when-let [task (get-in @state* [:pending id])]
    (deref task AWAIT-MS nil)))

(defn- connect-configured!
  "Connect, one at a time in id order, every committed server with no live
   client and no failure hold — a deterministic catalog and log order.
   connect-async! joins a connect already in flight rather than spawning."
  []
  (let [servers (:mcp (or (loader/snapshot "mcp await-connects") {}))]
    (doseq [id (sort (map name (keys servers)))
            :let [k (keyword id)]
            :when (and (not (live-client k)) (not (held? k (now-ms))))]
      (connect-async! k (server-config servers id))
      (await-pending! k))))

(defn await-connects!
  "Settle the catalog: connect every configured server not yet connected
   (or held after a failure), then block until every in-flight connect has
   landed (each bounded by AWAIT-MS). For hosts and tests that need a
   settled catalog; the turn path never calls it."
  []
  (connect-configured!)
  (doseq [id (keys (:pending @state*))]
    (await-pending! id)))
